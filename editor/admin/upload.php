...
$s_FormViewMode = InitSelect("d_viewmode", explode("|", "预览模式|列表模式"), explode("|", "|list"), $s_ViewMode, "", "onchange=\"location.href='?id=".$GLOBALS["sStyleID"]."&d_viewmode='+this.value+'&dir=".$GLOBALS["sDir"]."&page=".$sCurrPage."'\"");
...
echo $GLOBALS["sDir"]."/";}
echo $oDir."\">".$oDir."</a></td>".
"<td><a href='?id=".$GLOBALS["sStyleID"]."&d_viewmode=".$s_ViewMode."&dir=".$GLOBALS["sDir"]."&action=delfolder&foldername=".$oDir."'>删除</a></td></tr>";}}
if (isset($oFiles)){
...
if ($nCurrPage > 1) {
echo "<a href='?id=".$GLOBALS["sStyleID"]."&dir=".$GLOBALS["sDir"]."&d_viewmode=".$s_ViewMode."&page=1'>首页</a>&nbsp;&nbsp;<a href='?id=".$GLOBALS["sStyleID"]."&dir=".$GLOBALS["sDir"]."&d_viewmode=".$s_ViewMode."&page=".($nCurrPage - 1)."'>上一页</a>&nbsp;&nbsp;";}else{
echo "首页&nbsp;&nbsp;上一页&nbsp;&nbsp;";}
if ($nCurrPage < $i / $nPageSize) {
echo "<a href='?id=".$GLOBALS["sStyleID"]."&dir=".$GLOBALS["sDir"]."&d_viewmode=".$s_ViewMode."&page=".($nCurrPage + 1)."'>下一页</a>&nbsp;&nbsp;<a href='?id=".$GLOBALS["sStyleID"]."&dir=".$GLOBALS["sDir"]."&d_viewmode=".$s_ViewMode."&page=".$nPageNum."'>尾页</a>";}else{
echo "下一页&nbsp;&nbsp;尾页";}
echo "&nbsp;&nbsp;&nbsp;&nbsp;共<b>".$nFileNum."</b>个&nbsp;&nbsp;页次:<b><span class=highlight2>".$nCurrPage."</span>/".$nPageNum."</b>&nbsp;&nbsp;<b>".$nPageSize."</b>个文件/页";
echo "</td><td align=right><input type=checkbox id=b_selectall onclick='doCheckAll(this)'><label for=b_selectall>全选</label>&nbsp; <input type=submit name=b value='删除选定的文件'> <input type=button name=b1 value='清空本目录所有文件' onclick=\"javascript:if (confirm('你确定要清空所有文件吗？')) {location.href='?id=".$GLOBALS["sStyleID"]."&d_viewmode=".$s_ViewMode."&dir=".$GLOBALS["sDir"]."&action=delall';}\"></td></tr></table></td></tr>";}
echo "</form></table>";}
function DoDel(){
