...
function getMenuRow(s_Disabled, s_Event, s_Image, s_Html) {
	...
	s_MenuRow += ">"
	if (s_Image !=""){
		s_MenuRow += "&nbsp;<img border=0 src='images/button/"+s_Image+"' width=20 height=20 align=absmiddle "+s_Disabled+">&nbsp;";
	}else{
		s_MenuRow += "&nbsp;";
	}
...
function getTableMenuRow(what){
	...
	case "TableCell":
		...
		s_Menu += sMenuHr;
		s_Menu += getMenuRow(s_Disabled, "TableColInsertLeft()", "tablecoliinsertleft.gif", "插入列（在左侧）");
