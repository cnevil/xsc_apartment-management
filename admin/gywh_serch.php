...
$serch_tj=$_SESSION[gywh_serch];//修改查询的session值
if(isset($_GET[page])){
	$page=$_GET[page];
	}
else{
	...
	$query_page=mysql_query("SELECT * FROM `gywh` WHERE `gywh_title`='$serch_tj' OR `gywh_content`='$serch_tj' OR `gywh_time`='$serch_tj'");//修改取出的数据库
			...
			<div id="content">
				<table width="100%" height="25" border="1px" cellpadding="0" cellspacing="0" bordercolor="d6dbdf">
                	<tr bgcolor="e7ecf1">
                    	...
                    	<!--修改为本页面--><a href="gywh_serch.php?px=<?php
                    	if($_GET[px]==""){
							echo "up";
                    	}
                    	else if($_GET[px]=="up"){
						...
						$sql="SELECT * FROM `gywh` WHERE `gywh_title`='$serch_tj' OR `gywh_content`='$serch_tj' OR `gywh_time`='$serch_tj' ORDER BY `gywh_time` $sql_order LIMIT $start_page,$xsgs";//修改sql语句的值
                ...
                <table width="100%" height="25" border="1px" cellpadding="0" cellspacing="0" bgcolor="" bordercolor="#acadad">
                	...
                	<tr>
						...
						<td width="16%" align="center"><input id="cz" type="button" value="删除" onclick="var del=chackinfo();if(del){location.href='gywh.php?del=<?php echo $query_ip_row[gywh_id]?>'}else{ location.href='gywh.php'}"/><!--修改俩处del的值和取消的页面-->
