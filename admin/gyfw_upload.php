...
$db->Get_user_shell_check($_SESSION[id],$_SESSION[shell]);
 if(isset($_POST[sub]))
 {
    $sql = "UPDATE `gyfw` SET `gyfw_title` = '$_POST[ldjh_title]', `gyfw_content` = '$_POST[ldjh_content]', `gyfw_time` = '$_POST[ldjh_time]' WHERE `gyfw_id` = $_GET[upload];";//修改sql
    $query=mysql_query($sql);
    if($query)
    {
         echo "<script> alert(\"修改成功\"); location.href(\"gyfw.php\");</script>";
    }
    else
    {
       echo "<script> alert(\"修改失败\"); location.href(\"gyfw.php\");</script>";
    }
  }
...
<div id="main">
    ...
    <div id="middle_center">
		...
		<div id="main_content">
			...
			<div id="content">
				<?
                   $sql = "SELECT * FROM `gyfw` WHERE `gyfw_id`=$_GET[upload];";//sql
                   $query=mysql_query($sql);
                ...
                ?>
 				<table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
            	        <td height="30"><span id="add_upload">题目：&nbsp;&nbsp;&nbsp;<input type="text" name="ldjh_title" id="add_tm" value="<?php echo $row[gyfw_title]?>"/> </span></td>
                    </tr>
				    <tr>
                        <td><span id="add_upload">内容：</span>
                        	<textarea name="ldjh_content" id="text"><?php echo $row[gyfw_content]?></textarea>
                    ...
                    <tr>
                        <td height="30"><span id="add_upload">时间：&nbsp;&nbsp;&nbsp;<input type="text" onclick="setday(this)" name="ldjh_time" value="<?php echo $row[gyfw_time]?>"/></span></td>
                    </tr>
                    <tr>
