...
if(isset($_GET[page])){
	...
	$query_page=mysql_query("SELECT * FROM `tzgg`");//修改取出的数据库
	$num_rows = mysql_num_rows($query_page);
	$ys=$num_rows/14;
	$title_rows=ceil($ys);
	if(isset($_POST[sub])){
		$_SESSION[tzgg_serch]=$_POST[inp];//修改session的值
		header("Location:tzgg_serch.php");//修改链接的搜索页面

	}
if(isset($_GET[del])){
	$query = mysql_query("DELETE FROM  `tzgg` WHERE  `tzgg_id`=$_GET[del]");//修改取出的数据库

   if($query)
    {
	   echo "<script> alert(\"删除成功\"); location.href(\"tzgg.php\");</script>";//修改地址
			...
			<div id="content">
				<table width="100%" height="25" border="1px" cellpadding="0" cellspacing="0" bordercolor="d6dbdf">
                	<tr bgcolor="e7ecf1">
						...
						$sql="SELECT * FROM `tzgg` ORDER BY `tzgg_time` $sql_order LIMIT $start_page,$xsgs";//修改sql语句的值
                ...
                <table width="100%" height="25" border="1px" cellpadding="0" cellspacing="0" bgcolor="" bordercolor="#acadad">
                	...
                	<tr>
						...
						<td width="16%" align="center"><input id="cz" type="button" value="删除" onclick="var del=chackinfo();if(del){location.href='?del=<?php echo $query_ip_row[tzgg_id]?>'}else{ location.href='tzgg.php'}"/><!--修改俩处del的值和取消的页面-->
