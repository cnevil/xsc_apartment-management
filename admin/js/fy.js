...
showPages.prototype.createHtml = function(){ //生成html代码
	var strHtml = '', prevPage = this.page - 1, nextPage = this.page + 1;
			strHtml += '<div id="count">page: ' + this.page + ' / ' + this.pageCount + '</div>';
			strHtml += '<div id="number">';
			if (prevPage < 1) {
				strHtml += '<span title="第一页">&#171;</span>';
				strHtml += '<span title="前一页">&#139;</span>';
			} else {
				strHtml += '<span title="f"><a href="javascript:' + this.name + '.toPage(1);">&#171;</a></span>';
				strHtml += '<span title="prevPage"><a href="javascript:' + this.name + '.toPage(' + prevPage + ');">&#139;</a></span>';
			}
			if (this.page % 20 ==0) {
				var startPage = this.page - 19;
			} else {
				var startPage = this.page - this.page % 20 + 1;
			}
			if (startPage > 20) strHtml += '<span title="Prev 10 Pages"><a href="javascript:' + this.name + '.toPage(' + (startPage - 1) + ');">...</a></span>';
			for (var i = startPage; i < startPage + 20; i++) {
			...
			if (this.pageCount >= startPage + 20) strHtml += '<span title="Next 10 Pages"><a href="javascript:' + this.name + '.toPage(' + (startPage + 20) + ');">...</a></span>';
