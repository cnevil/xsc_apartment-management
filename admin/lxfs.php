...
if(isset($_POST[subfile])){
	$sc=$db->upfile('../images/','lxfs_xs.jpg',$_FILES["filename"],'image/pjpeg');//文件夹地址 文件名 文件本地地址 文件类型]
	if($sc){
		echo "<script> alert(\"修改成功\"); location.href(\"lxfs.php\");</script>";
	}
	else{
		echo "<script> alert(\"格式错误\"); location.href(\"lxfs.php\");</script>";
	}
}
?>
			...
			<div id="content_mima">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<form name="fileset" method="post"  enctype="multipart/form-data" action=""  onsubmit="return checkfile()">
                    	<tr>
                        	<td height="30"><span id="tb">上传图片：<input type="file"  name="filename"/> </span><span id="warn"></span></td>
                        </tr>
                        <tr>
