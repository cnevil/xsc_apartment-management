...
if (isset ($_POST[sub])) {
	$upload_sql = "UPDATE `zt` SET `zt_url` = '$_POST[lj_url]', `zt_name` = '$_POST[lj_name]' WHERE `id` = 1;";
	$upload_query = mysql_query($upload_sql);

	$img_name="zt_1.jpg";
    $img_ok = $db->upfile('../images/', $img_name, $_FILES["lj_img"], 'image/pjpeg'); //文件夹地址 文件名 文件本地地址 文件类型
	if ($upload_query) {
		echo "<script> alert(\"修改成功\"); location.href(\"zt_1.php\");</script>";
	} else {
		echo "<script> alert(\"修改失败\"); location.href(\"zt_1.php\");</script>";
	}
