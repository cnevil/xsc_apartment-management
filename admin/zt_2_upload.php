...
if (isset ($_POST[sub])) {
	$upload_sql = "UPDATE `zt` SET `zt_url` = '$_POST[lj_url]', `zt_name` = '$_POST[lj_name]' WHERE `id` = 2;";
	$upload_query = mysql_query($upload_sql);

	$img_name="zt_2.jpg";
    $img_ok = $db->upfile('../images/', $img_name, $_FILES["lj_img"], 'image/jpg'); //文件夹地址 文件名 文件本地地址 文件类型
	if ($upload_query) {
		echo "<script> alert(\"修改成功\"); location.href(\"zt_2.php\");</script>";
	} else {
		echo "<script> alert(\"修改失败\"); location.href(\"zt_2.php\");</script>";
	}
		...
		<div id="main_content">
			...
			<table width="100%" border="0" cellspacing="0"    cellpadding="0">
				<form name="psset" method="post" action=""  enctype="multipart/form-data" onsubmit="return checklj()">
                	<tr>
                    	<td height="30"><span id="user">图片名称：<input type="text"  name="lj_name" value="<?php echo $row[zt_name]?>"/> </span></td>
                    </tr>
					<tr>
                    	<td height="30"><span id="user">链接地址：<input type="text"  name="lj_url" value="<?php echo $row[zt_url]?>"/></span></td>
                    </tr>
                    <tr>
                        <td height="30"><span id="user">图片图片：<input type="file"  name="lj_img" /></span></td>
                    </tr>
                    <tr>
