...
if(isset($_GET[page])){
	...
	$query_page=mysql_query("SELECT * FROM `xjy`");//修改取出的数据库d
	...
	<div id="main_list_left">
			...
			<div id="main_list_left_middle">
				<div id="main_list_left_title">
					<ul>
							<?
							$query=mysql_query("SELECT * FROM `xjy` ORDER BY `xjy_time` desc  LIMIT $start_page, $xsgs");
							while($row=mysql_fetch_array($query)){
							?>
								<li><a href="xjy_content.php?id=<?php echo $row[xjy_id]?>"><?php echo $row[xjy_title]?></a></li>
							<?
							}
							?>
