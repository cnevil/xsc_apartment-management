...
if(isset($_GET[page])){
	...
	$query_page=mysql_query("SELECT * FROM `gyfw`");//修改取出的数据库d
...
<div id="main_list">
	<div id="main_list_left">
			<div id="main_list_left_top">
				您的位置：<a href="index.php">首页</a>=><a href="gyfw.php">公寓服务</a>
			</div>
			<div id="main_list_left_middle">
				<div id="main_list_left_title">
					<ul>
							<?
							$query=mysql_query("SELECT * FROM `gyfw` ORDER BY `gyfw_time` desc  LIMIT $start_page, $xsgs");
							while($row=mysql_fetch_array($query)){
							?>
								<li><a href="gyfw_content.php?id=<?php echo $row[gyfw_id]?>"><?php echo $row[gyfw_title]?></a></li>
							<?
							}
							?>
